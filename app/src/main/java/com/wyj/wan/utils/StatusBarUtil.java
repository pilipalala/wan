...
public final class StatusBarUtil {
    ...
    public static void setTranslucentForImageViews(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha,
                                                   View... needOffsetViews) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        setTransparentForWindow(activity);
        ...
        if (needOffsetViews != null && needOffsetViews.length > 0) {
            for (View needOffsetView : needOffsetViews) {
                Object haveSetOffset = needOffsetView.getTag(TAG_KEY_HAVE_SET_OFFSET);
                if (haveSetOffset != null && (Boolean) haveSetOffset) {
                    return;
                }
